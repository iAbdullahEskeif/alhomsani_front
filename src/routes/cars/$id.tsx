...
import type { Product, Review } from "@/types/interfaces.ts"
...
function ProductDetail() {
    ...
    useEffect(() => {
        const fetchSimilarCars = async () => {
            ...
            try {
                const token = await getToken()
                const response = await fetch(`${API_URL}/api/filtered/?car_type=${product.car_type}&limit=3`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                })
                ...
                const filteredCars = data.filter((car: Product) => car.id !== Number(id)).slice(0, 3)
    ...
    const toggleFavorite = async () => {
        ...
        try {
            const endpoint = isFavorite ? `/profiles/favorites/remove/${id}/` : `/profiles/favorites/add/${id}/`
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            ...
            const updatedReviewsResponse = await fetch(`${API_URL}/api/${id}/reviews/`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            })
        ...
        return (
            <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
                <Card className="max-w-md w-full bg-zinc-900 border-zinc-800 shadow-md">
                    ...
                    <CardContent>
                        <p className="text-zinc-400 mb-4">{error || "We couldn't find the car you're looking for."}</p>
                        <Button
                            asChild
                            variant="secondary"
    ...
    return (
        <div className="bg-zinc-950">
            <div className="w-full max-w-7xl mx-auto p-4 pt-6">
                <Link to="/" className="inline-flex items-center text-zinc-400 hover:text-amber-300 transition-colors">
                ...
                <Card className="bg-zinc-900 border-zinc-800 shadow-md overflow-hidden">
                    <CardContent className="p-8 md:p-12 flex flex-col md:flex-row items-center">
                        <div className="md:w-1/2 text-white mb-8 md:mb-0 z-10">
                            ...
                            <h1 className="text-3xl md:text-4xl lg:text-5xl font-medium mb-4 text-white">{product.name}</h1>
                            <div className="flex items-center mb-4">
                                <div className="flex items-center text-amber-400 mr-3">
                                    <Heart className={`size-5 mr-1 ${isFavorite ? "fill-amber-400" : ""}`} />
                                    <span>{/* {likeCount} {likeCount === 1 ? "person likes" : "people like"} this */}</span>
                                </div>
                            </div>
                            <p className="text-zinc-400 text-lg mb-6 border-l-4 border-amber-700/70 pl-4">{product.description}</p>
                            <div className="flex items-center">
                                <span className="text-3xl md:text-4xl font-medium text-white">{formatPrice(product.price)}</span>
                            ...
                            <div className="flex gap-4 mt-6">
                                <div className="flex items-center text-zinc-400">
                                    ...
                                    <span>{product.power ? product.power.split(" ")[0] : "N/A"}</span>
                                </div>
                                <div className="flex items-center text-zinc-400">
                                    <Clock className="size-5 text-amber-500 mr-2" />
                                    <span>{product.acceleration_0_100 ? product.acceleration_0_100.split(" ")[0] : "N/A"}s</span>
                                </div>
                                <div className="flex items-center text-zinc-400">
                        ...
                        <div className="md:w-1/2 flex justify-center z-10">
                            <div className="relative transition-transform duration-500 hover:scale-105">
                                <img
                                    ...
                                    onError={(e) => {
                                        e.currentTarget.src = "/placeholder.svg?height=400&width=600&text=No+Image"
                                    }}
                                />
                            </div>
                        </div>
            ...
            <div className="w-full max-w-7xl mx-auto p-4 mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    <Card className="bg-zinc-900 border-zinc-800 shadow-md">
                        <CardHeader>
                            <CardTitle className="text-2xl font-medium text-white">Performance Overview</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div className="flex flex-col items-center p-4 bg-zinc-800 rounded-lg">
                                    ...
                                    <span className="font-medium text-white">{product.power ? product.power.split(" ")[0] : "N/A"}</span>
                                </div>
                                <div className="flex flex-col items-center p-4 bg-zinc-800 rounded-lg">
                                    <Clock className="size-8 text-amber-500 mb-2" />
                                    <span className="text-sm text-zinc-500">0-100 km/h</span>
                                    <span className="font-medium text-white">
                                        {product.acceleration_0_100 ? product.acceleration_0_100 : "N/A"}
                                    </span>
                                </div>
                                <div className="flex flex-col items-center p-4 bg-zinc-800 rounded-lg">
                                    <Fuel className="size-8 text-amber-500 mb-2" />
                                    <span className="text-sm text-zinc-500">Fuel Economy</span>
                                    <span className="font-medium text-white">{product.fuel_economy ? product.fuel_economy : "N/A"}</span>
                                </div>
                                <div className="flex flex-col items-center p-4 bg-zinc-800 rounded-lg">
                                    <Calendar className="size-8 text-amber-500 mb-2" />
                                    <span className="text-sm text-zinc-500">Year</span>
                                    <span className="font-medium text-white">{new Date(product.created_at).getFullYear()}</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="bg-zinc-900 border-zinc-800 shadow-md">
                        <CardHeader>
                            <CardTitle className="text-2xl font-medium text-white">Key Features</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="grid grid-cols-1 gap-3">
                                {(showAllFeatures
                                    ...
                                    <div key={index} className="flex items-start bg-zinc-800 p-3 rounded-lg">
                    ...
                    <Card className="bg-zinc-900 border-zinc-800 shadow-md">
                        <CardHeader>
                            <CardTitle className="text-2xl font-medium text-white">Technical Specifications</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-3">
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        ...
                                        <span className="font-medium text-white">{product.engine || "N/A"}</span>
                                    </div>
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        <span className="text-zinc-500">Power</span>
                                        <span className="font-medium text-white">{product.power || "N/A"}</span>
                                    </div>
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        <span className="text-zinc-500">Torque</span>
                                        <span className="font-medium text-white">{product.torque || "N/A"}</span>
                                    </div>
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        <span className="text-zinc-500">Transmission</span>
                                        <span className="font-medium text-white">{product.transmission || "N/A"}</span>
                                    </div>
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                    ...
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        ...
                                        <span className="font-medium text-white">
                                            {product.wheelbase_mm ? `${product.wheelbase_mm} mm` : "N/A"}
                                        </span>
                                    </div>
                                </div>
                                <div className="space-y-3">
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        ...
                                        <span className="font-medium text-white">{product.acceleration_0_100 || "N/A"}</span>
                                    </div>
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        <span className="text-zinc-500">Top Speed</span>
                                        <span className="font-medium text-white">{product.top_speed || "N/A"}</span>
                                    </div>
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        <span className="text-zinc-500">Fuel Economy</span>
                                        <span className="font-medium text-white">{product.fuel_economy || "N/A"}</span>
                                    </div>
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        <span className="text-zinc-500">Dimensions</span>
                                        <span className="font-medium text-white">{product.dimensions || "N/A"}</span>
                                    </div>
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        <span className="text-zinc-500">Fuel Tank</span>
                                        <span className="font-medium text-white">
                                            {product.fuel_tank_capacity ? `${product.fuel_tank_capacity} liters` : "N/A"}
                                        </span>
                                    </div>
                                    <div className="flex justify-between pb-2 border-b border-zinc-800">
                                        <span className="text-zinc-500">Trunk Capacity</span>
                                        <span className="font-medium text-white">
                                            {product.trunk_capacity_liters ? `${product.trunk_capacity_liters} liters` : "N/A"}
                                        </span>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="space-y-8">
                    <Card className="sticky top-5 bg-zinc-900 border-zinc-800 shadow-md">
                        ...
                        <CardContent className="space-y-6">
                            <div>
                                <h3 className="text-sm font-medium text-zinc-500 mb-2">Stock & Availability</h3>
                                <div className="flex items-center">
                                    <span
                                        className={`inline-block size-3 rounded-full ${product.availability === "in_stock" ? "bg-emerald-500" : "bg-zinc-500"
                                            } mr-2`}
                                    ></span>
                            ...
                            <div>
                                <h3 className="text-sm font-medium text-zinc-500 mb-2">Price Breakdown</h3>
                                <div className="space-y-2">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className="text-white">${destinationFee.toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-zinc-500">Est. Tax & Title</span>
                                        <span className="text-white">${taxAndTitle.toLocaleString()}</span>
                                    </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <Dialog open={showStripeModal} onOpenChange={setShowStripeModal}>
                                    <DialogContent className="sm:max-w-md bg-zinc-900 border-zinc-800 text-zinc-200">
                                        <DialogHeader>
                                            <DialogTitle className="text-zinc-200">Complete Your Purchase</DialogTitle>
                                        </DialogHeader>
            ...
            <div className="w-full max-w-7xl mx-auto p-4 mt-4">
                <Card className="bg-zinc-900 border-zinc-800 shadow-md">
                    <CardHeader className="flex flex-row items-center justify-between">
                        <CardTitle className="text-2xl font-medium text-white">Customer Reviews</CardTitle>
                        <Button
                            variant="ghost"
                        ...
                        <div className="flex flex-col md:flex-row gap-8">
                            <div className="md:w-1/3">
                                <div className="flex flex-col items-center">
                                    ...
                                    >
                                        <Heart className={`mr-2 h-4 w-4 ${isFavorite ? "fill-amber-400" : ""}`} />
                            ...
                            <div className="md:w-2/3">
                                {reviews.length > 0 ? (
                                                        ...
                                                        <div className="flex items-center">
                                                            ...
                                                            <div className="flex-shrink-0 mr-2">
                                                                {review.reviewer_Profile_pic !== null ? (
                                                                    <img
                                                                        src={review.reviewer_Profile_pic || "/placeholder.svg"}
                                                                        alt="Profile"
                                                            ...
                                                            >
                                                                <Link
                                                                    to="/profile/stalk/$id"
                                                                    params={{ id: review.reviewer_id ? String(review.reviewer_id) : review.reviewer }}
                                                                >
                                                                    {review.reviewer}
                                                                </Link>
                                                            </Button>
                                                        </div>
                                                        <div className="text-zinc-500 text-sm">{formatReviewDate(review.time_written)}</div>
                                                    </div>
                                    ...
                                    <div className="text-center py-8">
                                        ...
                                        <h3 className="text-zinc-400 text-lg mb-2">No Reviews Yet</h3>
                                        <p className="text-zinc-500 mb-4">Be the first to share your experience with this vehicle</p>
                                        <Button
                                            variant="outline"
                                ...
                                {reviews.length > 5 && (
                                    <Button variant="ghost" className="mt-6 text-zinc-400 hover:text-amber-300 hover:bg-zinc-800">
            ...
            <div className="w-full max-w-7xl mx-auto p-4 mt-8 mb-16">
                <h2 className="text-2xl font-medium text-white mb-6">You May Also Like</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {similarCars.length > 0 ? (
                        similarCars.map((car) => (
                            ...
                            >
                                ...
                                <CardContent className="p-4">
                                    <h3 className="font-medium text-lg mb-1 text-white">{car.name}</h3>
                                    <p className="text-zinc-500 mb-3">Starting at {formatPrice(car.price)}</p>
                                    <Button
                                        variant="ghost"
                                        asChild
                        ...
                        <div className="col-span-3 text-center py-8">
                            <p className="text-zinc-500">No similar vehicles available at this time.</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <footer className="w-full bg-zinc-950 border-t border-zinc-800 py-8">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="text-center text-zinc-600 text-sm">
                        ...
                        <p className="mt-2">Engineered for those who drive the extraordinary.</p>
                    </div>
                </div>
            </footer>
        </div>
