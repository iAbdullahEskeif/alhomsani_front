...
import { fetchCars } from "@/services/api/fetchCars";
import { fetchProfile } from "@/services/api/fetchProfiles";
import {
    MapPin,
    Mail,
    Calendar,
...
import { formatTime, formatDate, getActionText, getActionIcon, getCarNameFromActivity } from "@/utils/activityHelpers";
...
function ProfilePage() {
    ...
    const { data: favoriteCarsData = [], isLoading: isFavoritesLoading } =
        useQuery<Product[]>({
            ...
            queryFn: () => fetchCars(profile?.favorite_cars || [], getToken, API_URL),
    ...
    const { data: bookmarkedCarsData = [], isLoading: isBookmarksLoading } =
        useQuery<Product[]>({
            ...
            queryFn: () => fetchCars(profile?.bookmarked_cars || [], getToken, API_URL),
    ...
    const {
        ...
        queryFn: () => fetchProfile(getToken, API_URL),
    ...
    const toggleFavorite = async (carId: number, isFavorite: boolean) => {
        ...
        try {
            ...
            if (!isFavorite) {
                ...
                const carData = allCars.find((c: Product) => c.id === carId);
                                                            ...
                                                            <div className="flex justify-between items-start">
                                                                <div>
                                                                    ...
                                                                    <p className="text-amber-500 text-sm">
                                                                        Car:{" "}
                                                                        {getCarNameFromActivity(activity.product, carNames)}
                                                                    </p>
                                                                </div>
